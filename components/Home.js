...
const { truncateString } = require('../modules/truncateString');
...
// test
  useEffect(() => {
    fetch('https://mymoviz-backend-sable.vercel.app/movies')
      .then(response => response.json())
      .then(data => {
        // Voir la sortie Thunder Client
        setMoviesData(data.movies);
        console.log(data.movies[0].voteCount)
      });
  }, []);
...
let url = "https://image.tmdb.org/t/p/original"
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={truncateString(data.overview)} poster={url.concat(data.poster_path)} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
